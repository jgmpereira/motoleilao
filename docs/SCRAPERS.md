...
**Correção (Jul/2026): colisão de número de lote entre dois auction_ids reais no mesmo `leilao_id`.** `gerarLeilaoId()` (`sodre.js`) gera o `leilao_id` só pela data — se a Sodré roda dois leilões reais distintos (auction_ids diferentes) no mesmo dia, os dois caem no mesmo `leilao_id` nosso, mas `leiloes.link` só guarda um auction_id. `sodre-encerrados.js` buscava `lots-finished` só desse auction_id do link e casava lote→moto só por número, ignorando de qual auction_id real cada moto vinha — se dois lotes de auction_ids diferentes tinham o mesmo número, o valor de um vazava pro outro. Caso real: BMW F750 GS (`sodre_2026_07_14`, auction_id 28787, lote 32, R$40.000) recebeu o valor de um Fiat Uno de outro leilão real do mesmo dia (auction_id 28765, lote 32 também, R$18.000). Corrigido: agora extrai o auction_id de cada moto individualmente via `motos.url`, agrupa por auction_id e só casa lote↔moto dentro do mesmo auction_id. Também ampliada a janela de reprocessamento incondicional de 3 para 7 dias, dando mais folga pra qualquer leilão recente ser reconferido.
